...
*/
   ...
   private static String listaCampos(){
	    	...
	    	while (i<data.getAtributos().size()) {
	    		if( data.getAtributos().get(i).check){
		    		...
		    		if (result.compareTo("")==0){
		    			result = data.getAtributos().get(i).nombre;
		    		}else {
		    			result = result + ", ";
    ...
    /**Esta funcion retorna un Booleano que representa si existe alguna clave repetida
     * para esta entidad con nombre 'name'
     * @param String name
     * @return Boolean
     */
     ...
     * El parametro de entrada 'a' significa si el insert esta correcto o no
     * Si el insert esta correcto, se escribira en 'insert.sql'
     * Si el insert esta incorrecto, se escribira en 'bad.insert.sql'
     * @param int a
     */
    ...
    /** Procedimiento que se encarga de escribir la primera linea del archivo en los archivos
     * que se deben inicializar al inicio del Parser: ejemplo 'bad.xml's
     */
    public static void iniciarXmlError(){
		try{ xmlError.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
		}catch (Exception e){System.err.println("Error: " + e.getMessage());}
	}
    /** SobreEscribirel Procedimiento 'startDocument' esto nos permite utilizar los eventos que 
     * nos porporciona el Parser y la Libreria SAX
     */
    @Override
    ...
    /** SobreEscribirel Procedimiento 'endDocument' esto nos permite utilizar los eventos que 
     * nos porporciona el Parser y la Libreria SAX
     */
    @Override
    public void endDocument() {System.out.println("\n</XML>\n");}
    
    /** SobreEscribirel Procedimiento 'startElement' esto nos permite utilizar los eventos que 
     * nos porporciona el Parser y la Libreria SAX
     * En especifico, este procedimiento crea los objetos OrigenXML(en caso de ser Entidades)
     * donde se almacenan las respectivas entidades, atributos y valores que se estaran
     * utilizando; o detectara que son atributos y los alamacenara en la respectiva entidad
     */
    @Override
...
//System.out.print("*"+nivelActual);
    	if (errorFound){
	    	...
	    	if ( buscarEntidadB(name) ){			 		//Si el tag es una entidad
	    		...
	    		nivelActual++;	      					  	//sumar nivel actual
	   			...
	   			//si existen atributos dentro del tag	
	   			if (atts.getLength()!=0){
	   				int t = atts.getLength();
	   				int t1= 0;
	   				while (t1< t){
	   					System.out.println("\n["+t1+"]"+ atts.getLocalName(t1)+ " = " + atts.getValue(t1));
	   					t1++;
	   				}
	   			}
	   		}else {		 									
    ...
    /** SobreEscribirel Procedimiento 'endElement' esto nos permite utilizar los eventos que 
     * nos porporciona el Parser y la Libreria SAX
     * En especifico, este procedimiento nos permite decidir si el insert es conveniente 
     * escribirlo en el archivo correspondiente
     */
    @Override
